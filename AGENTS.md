...
- **無音候補レビュー**：検出後に「無音候補 N/総数」を1件ずつ確認。前へ/次へ/試聴/採用して削除/
  ...
  しない。採用は「削除区間リスト」の✕、または再度「採用を取り消す」/スキップで取り消せる。旧候補
  一覧は折りたたみで残してあり、レビューUIが操作の本流。
- **削除判断の唯一の真実は無音候補の `status`（pending/adopted/skipped）**。採用/スキップ/取り消しは
  `status` を書き換えるだけで、実際に削除される区間は `effectiveCuts()`（手動 `cuts` ∪ 採用済み候補）
  として都度導出する。`cuts` には手動削除のみを入れ、無音採用分を二重に持たせないこと（状態のズレ防止）。
